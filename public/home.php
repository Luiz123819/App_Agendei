...
// 2. Resgata os dados da sessão (agora salvos corretamente no login)
$nomeUsuario = $_SESSION['nome_user'] ?? 'Visitante';
$emailUsuario = $_SESSION['email_user'] ?? '';

// 3. Lógica do Gravatar - Forçando limpeza de espaços e minúsculas
...
// 4. Busca os médicos (seu código atual)
try {
...
<nav class="nav-bar" style="display: flex; justify-content: space-between; align-items: center; padding: 15px; background: #007bff; color: white;">
    <div class="test-nav">
        <h2 style="margin:0;">Olá, <?= htmlspecialchars($nomeUsuario) ?>!</h2>
        <span>Como podemos ajudar hoje?</span>
    </div>
    <div class="user-photo">
        <img src="<?= $urlGravatar ?>" alt="Foto de <?= htmlspecialchars($nomeUsuario) ?>" 
             style="border-radius: 50%; width: 50px; height: 50px; object-fit: cover; border: 2px solid #fff;">
    </div>
</nav>
